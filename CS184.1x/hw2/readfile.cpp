        ...
        if (cmd == "light") { 
          if (numused == numLights) // No more Lights 
            ...
            if (validinput) {
			  ...
			  const float r = values[4];
			  const float g = values[5];
			  const float b = values[6];
			  const float a = values[7];
        ...
        // Note that no transforms/stacks are applied to the colors. 
          ...
          if (validinput) {
            ...
            // Set eyeinit upinit center fovy in variables.h 
		      ...
		      vec3 eyecoords(values[0],values[1],values[2]);
			  vec3 cencoords(values[3],values[4],values[5]);
			  vec3 upcoords(values[6],values[7],values[8]);
			  
			  eyeinit = eyecoords;
			  center = cencoords;
			  upinit = Transform::upvector(upcoords, eyeinit);
			  fovy = values[9];
			}
        }
        ...
        // you can get a sense of how this works.  
          ...
          if (validinput) {
            ...
            // You might want to use helper functions on top of file.
			  ...
			  const float tx = values[0];
			  const float ty = values[1];
			  const float tz = values[2];
			  
			  mat4 trans_mat = Transform::translate(tx,ty,tz);
			  
			  rightmultiply(trans_mat,transfstack);
          }
        }
        else if (cmd == "scale") {
          validinput = readvals(s,3,values) ; 
          if (validinput) {
            ...
            // You might want to use helper functions on top of file.
			  ...
			  const float sx = values[0];
			  const float sy = values[1];
			  const float sz = values[2];
			  
			  mat4 scale_mat = Transform::scale(sx,sy,sz);
          ...
          if (validinput) {
            ...
            // See how the stack is affected, as above.  
			  ...
			  mat3 _rot_mat = Transform::rotate(values[3], axis);
			  mat4 rot_mat = mat4(_rot_mat); 
